...
model {
  ...
  for(n in 1:N){
    for(k in 1:K){
      eta[n,k] =B[k]*time[n] +  dot_product(other_cov[n], phi[k]) +  ran_eff[ids[n]][1,k] + ran_eff[ids[n]][2,k]*time[n]; //evolution of the latent trajectories  + dot_product(phi[k,], other_cov[ids[n],])
    }
  }
  y_mu = eta*Lambda';
  for(n in 1:N){
...
generated quantities{
  ...
  hazard_pbo = exp(-b0*c);
  hazard_ratio_pa = exp((b_rf[2,2])*c);
  hazard_ratio_na = exp(b_rf[2,1]*c);
